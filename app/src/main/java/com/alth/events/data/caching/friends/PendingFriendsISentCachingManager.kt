...
class PendingFriendsISentCachingManager @Inject constructor(
    ...
    suspend fun updateLocalFriends(
        ...
        val response = networkFriendshipDataSource.getFriendsIRequested(
            page = query.page,
        ...
        when (response) {
            is NetworkResult.Success -> {
                val friendships = response.t.map {
                    FriendshipEntity.from(it, FriendType.PendingSentFromMe)
                }
                transactionUseCase {
                    if (invalidateCache) {
                        friendshipLocalDataSource.clearPendingSentFromMe()
                    }
