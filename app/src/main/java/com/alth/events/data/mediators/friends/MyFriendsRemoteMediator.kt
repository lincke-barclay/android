...
class MyFriendsRemoteMediator @Inject constructor(
    ...
    override suspend fun load(
        ...
        val nextPage = when (loadType) {
            ...
            LoadType.APPEND -> {
                ...
                val numLoaded = friendshipLocalDataSource.getNumberAccepted()
                numLoaded / state.pages.size
            }
        }

        return when (val response = myConfirmedFriendsCachingManager.updateLocalFriends(
            query = FriendsQuery(
                page = nextPage,
