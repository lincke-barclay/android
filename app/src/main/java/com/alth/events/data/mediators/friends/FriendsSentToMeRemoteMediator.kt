...
import com.alth.events.data.caching.friends.PendingSentToMeCachingManager
...
class FriendsSentToMeRemoteMediator @Inject constructor(
    private val friendsSentToMeCachingManager: PendingSentToMeCachingManager,
    ...
    override suspend fun load(
        ...
        val nextPage = when (loadType) {
            ...
            LoadType.APPEND -> {
                ...
                val numLoaded = friendshipLocalDataSource.getNumberPendingSentToMe()
                numLoaded / state.pages.size
            }
        }

        return when (val response = friendsSentToMeCachingManager.updateLocalFriends(
            query = FriendsQuery(
                page = nextPage,
